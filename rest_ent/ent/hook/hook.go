...
// The CateFunc type is an adapter to allow the use of ordinary
// function as Cate mutator.
type CateFunc func(context.Context, *ent.CateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.CateMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CateMutation", m)
	}
	return f(ctx, mv)
}
